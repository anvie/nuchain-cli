...
import {Logger} from "@nuchain-cli/core/dist/logger";
...
async function fetchFromWebService(log: TsLogger): Promise<Map<AccountId, Array<Contributor>>> {
    ...
    try {
        let response = await axios({
            url: "https://app.gc.subsquid.io/beta/nuchain-sqd5/v2/graphql",
            method: "post",
...
async function transformIntoRewardee(
    ...
    const codes = await Crowdloan.fetchCodeAddressPairs('nuchain', sqlConfig);
